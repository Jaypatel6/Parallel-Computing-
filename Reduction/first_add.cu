...
kernel3(dtype *g_idata, dtype *g_odata, unsigned int n)
{
	...
	if(i < n) {
		scratch[threadIdx.x] = g_idata[i] + g_idata[i + blockDim.x]; 	// add while loading to shared array
	} else {
		scratch[threadIdx.x] = 0;
	}
	__syncthreads ();

	// blockDim is half of that of Sequential
	for(int stride = (blockDim.x/2); stride >= 1; stride = (stride >> 1)) {	//stride length halves in each iteration
