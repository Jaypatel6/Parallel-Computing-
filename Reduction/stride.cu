...
kernel1(dtype *input, dtype *output, unsigned int n)
{
	...
	for(unsigned int s = 1; s < blockDim.x; s = s << 1) {
		int slot = (s * 2) * threadIdx.x;			// calculate index for every even threadIdx
		if(slot < blockDim.x) {				// check index range		
			scratch[slot] += scratch[slot + s];
		}
		__syncthreads ();
  	}
...
main(int argc, char** argv)
{
  ...
  fprintf (stdout, "Time to execute strided index GPU reduction kernel_Part_2: %Lg secs\n", t_kernel_1);
